...
const getAndWrapConstraintsForService = async (serviceId, roles, correlationId, clients, caller = false, selectedRoleIds = false) => {
  ...
  const roleSelectionConstraint = serviceParams['roleSelectionConstraint'];
  if (roleSelectionConstraint && caller && selectedRoleIds) {
    selectedRoleIds.map(role => {
      let restrictedRole = serviceParams[role]
      if (restrictedRole && selectedRoleIds.includes(restrictedRole)) {
        // Only accommodates A OR B but not complex scenarios
        let firstRestrictedRole = roles.filter(rid => rid.id == role).map(obj => obj.name);
        let scondRestrictedRole = roles.filter(rid => rid.id == restrictedRole).map(obj => obj.name);
        constraints.push(new SelectionConstraint(firstRestrictedRole[0], scondRestrictedRole[0]));
      }
    })
  }
...
class PolicyEngine {
  ...
  async validate(userId, organisationId, serviceId, selectedRoleIds, correlationId) {
    const roles = await this._clients.access.getRolesForService(serviceId, correlationId);
    const constraints = await getAndWrapConstraintsForService(serviceId, roles, correlationId, this._clients, true, selectedRoleIds);
