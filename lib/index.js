...
const buildUserDetails = async (clients, userId, organisationId, serviceId, correlationId) => {
  ...
  console.timeEnd('    check-access-to-org');
  console.time('    get-user-access-to-service-at-org');
  ...
  console.timeEnd('    get-user-access-to-service-at-org');
...
const applyPoliciesToUser = (user, policies, constraints) => {
  ...
  console.time('  extract fields from user')
  const userFields = extractFieldsFromUser(user);
  console.timeEnd('  extract fields from user')

  console.time('check-all-policy-conditions-fulfilled');
  policies.forEach((policy) => {
    console.time('check-one-policy-condition-fulfilled');
    ...
    console.timeEnd('check-one-policy-condition-fulfilled');
  });
  console.timeEnd('check-all-policy-conditions-fulfilled');

  console.time('check-has-changed');
  let hasChanged = true;
  while (hasChanged) {
    hasChanged = false;
  ...
  console.timeEnd('check-has-changed');
  console.log('- end of get and apply policies to user');
...
const getAndApplyPoliciesToUser = async (userId, organisationId, serviceId, correlationId, roles, constraints, clients) => {
  ...
  console.log('  -- calling: build user details');
  console.time('    build-user-details');
  const user = await buildUserDetails(clients, userId, organisationId, serviceId, correlationId);
  console.log('  -- end of build user details');
  console.timeEnd('    build-user-details');
...
class PolicyEngine {
  ...
  async validate(userId, organisationId, serviceId, selectedRoleIds, correlationId) {
    ...
    const selectedRolesNotAvailableToUser = selectedRoleIds.filter(rid => !policyResult.rolesAvailableToUser.find(r => r.id.toLowerCase() === rid.toLowerCase()));
