...
const getAndWrapConstraintsForService = async (serviceId, roles) => {
  const constraints = [
    new MinimumConstraint(1),
    new MaximumConstraint(2),
    new ParentChildConstraint(roles),
  ];
  return {
    constraints,
...
class PolicyEngine {
  ...
  async validate(userId, organisationId, serviceId, selectedRoleIds, correlationId) {
    ...
    const constraints = await getAndWrapConstraintsForService(serviceId, roles);
