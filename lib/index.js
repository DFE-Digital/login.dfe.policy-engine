...
const getAndWrapConstraintsForService = async (serviceId, roles, correlationId, clients, selectedRoleIds) => {
  ...
  if (roleSelectionConstraint && selectedRoleIds) {
    roleSelectionConstraint = roleSelectionConstraint.split(',').map( (role) => role.trim());
    if (selectedRoleIds.includes(roleSelectionConstraint[0]) && selectedRoleIds.includes(roleSelectionConstraint[1]) ) {
      let firstRestrictedRole = roles.filter(rid => rid.id == roleSelectionConstraint[0]).map(obj => obj.name);
      let scondRestrictedRole = roles.filter(rid => rid.id == roleSelectionConstraint[1]).map(obj => obj.name);
...
class PolicyEngine {
  ...
  async validate(userId, organisationId, serviceId, selectedRoleIds, correlationId) {
    const roles = await this._clients.access.getRolesForService(serviceId, correlationId);
    const constraints = await getAndWrapConstraintsForService(serviceId, roles, correlationId, this._clients, selectedRoleIds);
