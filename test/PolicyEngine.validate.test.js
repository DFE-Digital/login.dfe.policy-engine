...
describe("when validating selected roles", () => {
  ...
  beforeEach(() => {
    ...
    getServicePoliciesRaw.mockReset().mockResolvedValue([
      {
        id: "policy-1",
        name: "policy one",
    ...
    SelectionConstraint.mockReset().mockImplementation(
      () => selectionConstraint,
    );
  ...
  it("then it should return exceptions if a role selected is not available through policies", async () => {
    const badSelectedRoles = [
      allServiceRoles[0].id,
      "not-a-role-1",
      "not-a-role-2",
    ];
  ...
  it("then it should return exceptions if minimum constraint fails", async () => {
    const constraintError = {
      ...
      message:
        "Expected a minimum of X roles to be selected but received X selected roles",
  ...
  it("then it should return exceptions if maximum constraint fails", async () => {
    const constraintError = {
      ...
      message:
        "Expected a maximum of X roles to be selected but received X selected roles",
      appliesTo: selectedRoleIds,
    };
  ...
  it("then it should apply parent/child constraint if configured for service", async () => {
    ...
    expect(parentChildConstraint.validate).toHaveBeenCalledWith(
      selectedRoleIds,
    );
  });
  ...
  it("then it should return exceptions if all constraints fails", async () => {
    ...
    parentChildConstraint.validate.mockReturnValue([
      parentChildConstraintError,
    ]);
  ...
  it("then it should apply Selection constraint if configured for service", async () => {
      ...
      relyingParty: {
        params: {
          roleSelectionConstraint:
            allServiceRoles[0].id + "," + allServiceRoles[1].id,
        },
      },
    });
    await engine.validate(
    ...
    expect(SelectionConstraint).toHaveBeenCalledWith(
      allServiceRoles[0].name,
      allServiceRoles[1].name,
    );
